...
class ClearMessages(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.logger = get_logger("메시지 정리", bot=self.bot)
    ...
    async def clear(self, interaction: discord.Interaction, amount: int):
        ...
        if not interaction.channel.permissions_for(interaction.user).manage_messages:
            # Get server-specific log channel for logging
            log_channel_id = get_channel_id(interaction.guild.id, 'log_channel')
            if log_channel_id:
                self.logger.info(
        ...
        try:
            ...
            if log_channel_id:
                self.logger.error(
                    f"❌ Bot lacks permissions to delete messages in #{interaction.channel.name} ({interaction.channel.id}) "
                    f"(Guild: {interaction.guild.id}): Forbidden.\n{traceback.format_exc()}"
                )
            ...
            if log_channel_id:
                self.logger.error(
                    f"❌ HTTP error during message purge in #{interaction.channel.name} ({interaction.channel.id}) "
            ...
            if log_channel_id:
                self.logger.critical(
                    f"❌ Unknown error during message purge in #{interaction.channel.name} ({interaction.channel.id}) "
                    f"(Guild: {interaction.guild.id}): {e}\n{traceback.format_exc()}",
                    exc_info=True
                )
